...
export default (container) => {
    container.innerHTML = `
        <div class="tool-form">
            <p style="color:var(--text-secondary); margin-bottom:1rem;">Convert any audio file format playable in your browser (MP3, AAC, OGG, FLAC) into high-fidelity uncompressed WAV format.</p>
            <label for="audioConvFile">Select Audio File:</label>
            <input type="file" id="audioConvFile" accept="audio/*">
            <button id="audioConvButton" style="margin-top:1.2rem;">🎵 Convert to WAV & Download</button>
            <audio id="audioConvPreview" controls style="width:100%; margin-top:1rem; display:none;"></audio>
        </div>
    `;
                ...
                try {
                    ...
                    ctx.decodeAudioData(event.target.result)
                        .then(buffer => { 
                            ...
                            showAlert('Audio loaded successfully. Ready to convert to WAV.', 'success');
                        })
                        .catch(err => showAlert(`Error decoding audio: ${err.message}. Try a standard MP3, OGG, or AAC file.`, 'error'));
                } catch (err) {
    ...
    convertButton.onclick = () => {
        ...
        try {
            ...
            a.download = `${safeName}_converted.wav`;
